...
// ─── Default subjects (used only if user has no saved subjects) ───────────────
...
// ─── Provider to hold user's subject list in memory ──────────────────────────
...
class _AddEditNoteScreenState extends ConsumerState<AddEditNoteScreen> {
  ...
  void initState() {
    ...
    // Set initial subject — use note's subject if editing, else first in list
    final subjects = ref.read(userSubjectsProvider);
    if (widget.note != null) {
      _subject = widget.note!.subject;
      _subjectColor = widget.note!.subjectColor;
      // Add the note's subject to the list if it's a custom one not in list
      if (!subjects.containsKey(_subject)) {
        ref.read(userSubjectsProvider.notifier).state = {
          ...subjects,
          _subject: _subjectColor,
        };
      }
    } else {
  ...
  // ─── Show bottom sheet to add a new custom subject ──────────────────────
  ...
  // ─── Show bottom sheet to manage (delete) existing subjects ─────────────
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () {
                              ...
                              ref
                                  .read(userSubjectsProvider.notifier)
                                  .state = updated;
                              // If deleted subject was selected, switch to first
  ...
  Widget build(BuildContext context) {
    ...
    // Make sure _subject is still valid if subjects changed
    if (!subjects.containsKey(_subject)) {
      _subject = subjects.keys.first;
      _subjectColor = subjects.values.first;
    }
